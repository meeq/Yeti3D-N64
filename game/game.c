...
void player_tick(yeti_t* yeti, entity_t* e)
{
  ...
  else
  {
    ...
    if (yeti->keyboard.state.cu     ) e->p -= i2f(2);
    if (yeti->keyboard.state.cd     ) e->p += i2f(2);
    if (yeti->keyboard.state.select )
    {
      if (yeti->keyboard.state.left   ) { entity_turn_left(e); entity_turn_left(e); }
      if (yeti->keyboard.state.right  ) { entity_turn_right(e); entity_turn_right(e); }
      if (yeti->keyboard.state.up     ) { entity_move_forward(e); entity_move_forward(e); }
      if (yeti->keyboard.state.down   ) { entity_move_backwards(e); entity_move_backwards(e); }
      if (yeti->keyboard.state.l      ) { entity_move_left(e); entity_move_left(e); }
      if (yeti->keyboard.state.r      ) { entity_move_right(e); entity_move_right(e); }
    }
    else
    {
      ...
      if (yeti->keyboard.state.l      ) entity_move_left(e);
      if (yeti->keyboard.state.r      ) entity_move_right(e);
    }
    ...
    if (e->state.pain)
    {
      if (yeti->game.hud.health > e->state.pain)
      {
        yeti->game.hud.health -= e->state.pain;
        e->state.pain = 0;
      }
      else
      {
...
{
  e->radius = 128; // 0 allows walking through walls
...
{
  // gives a boost at the start of each level - if needed
  if (hud->health < 100)
    hud->health = 100;
  if (hud->ammo < 150)
    hud->ammo = 150;
...
void game_tick(yeti_t* yeti)
{
  switch (game_mode(&yeti->game))
  {
    ...
    {
      yeti_tick(yeti);
      if (KEYPRESS_START(yeti)) game_goto(&yeti->game, GAME_MODE_PAUSED);
      break;
    }
    ...
    {
      if (KEYPRESS_START(yeti)) game_goto(&yeti->game, GAME_MODE_PLAY);
      if (KEYPRESS_A (yeti))
      {
    ...
    case GAME_MODE_ENDLEVEL:
    {
      ...
      {
        yeti->game.episode++;
        if (yeti->game.episode >= GAME_EPISODE_MAX) yeti->game.episode = 0;
        game_goto(&yeti->game, GAME_MODE_START_LEVEL);
      }
      break;
    }
