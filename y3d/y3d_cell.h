...
typedef struct rom_cell_t
{
  ...
  s16 top __attribute__((packed));  // 8:8 fixed point cell top value.
  u8  wtb;  // Cell wall bot texture id.
  u8  sw2;  // Cell switch extra.
  s16 bot __attribute__((packed));  // 8:8 fixed point cell bot value.
  s16 lit __attribute__((packed));  // Cell light setting.
