  ...
  int mode)
{
  ...
  if (w > 0 && h > 0 && w < YETI_RECIPROCAL_MAX && h < YETI_RECIPROCAL_MAX)
  {
    ...
    if (x1 < YETI_VIEWPORT_X1) {u1 += (YETI_VIEWPORT_X1 - x1) * uu; x1 = YETI_VIEWPORT_X1;}
    if (y1 < YETI_VIEWPORT_Y1) {v1 += (YETI_VIEWPORT_Y1 - y1) * vv; y1 = YETI_VIEWPORT_Y1;}
    if (x2 > YETI_VIEWPORT_X2) {x2 = YETI_VIEWPORT_X2;}
    if (y2 > YETI_VIEWPORT_Y2) {y2 = YETI_VIEWPORT_Y2;}
