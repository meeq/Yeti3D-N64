...
void map_init_lod(map_t* map, int groupsize)
{
  ...
  for (y1 = 1; y1 < map->height - 1; y1++)
  {
    for (x1 = 1; x1 < map->width - 1; x1++)
    {
      ...
      if (size > 0) map_group(map, r.x1, r.y1, r.x2, r.y2);
    }
  }
}
...
void map_draw_light(map_t* map, int lightx, int lighty)
{
  ...
  for (i = 0; i < DEG_360; i += DEG_1)
  {
    ...
    for (lit = 64; lit > 0;)
    {
      x += xx; if ((xhit = CELL_IS_SOLID(&map->item[f2i(y)][f2i(x)])) != FALSE) xx = -xx;
      y += yy; if ((yhit = CELL_IS_SOLID(&map->item[f2i(y)][f2i(x)])) != FALSE) yy = -yy;
      cell = &map->item[f2i(y + FIXHALF)][f2i(x + FIXHALF)];

      cell->lit = MIN(cell->lit + lit, CELL_LIGHT_MAX);

      if (xhit || yhit) break;//lit >>= 2;
    }
  }
}
...
void map_init_slopes(map_t* map)
{
  ...
  for (y = 0; y < map->height; y++)
  {
    for (x = 0; x < map->width; x++)
    {
      ...
      for (i = 1; i < 4; i++)
      {
        if (c->bots[i] > botmax) botmax = c->bots[i];
        if (c->bots[i] < botmin) botmin = c->bots[i];
        if (c->tops[i] > topmax) topmax = c->tops[i];
        if (c->tops[i] < topmin) topmin = c->tops[i];
      }
...
int map_line_of_sight(map_t* map, int x1, int y1, int x2, int y2)
{
    ...
    y2 >= 0 && y2 < i2f(map->height) )
  {
    ...
    y2 -= y1;
    {
      ...
      for (; i--; x1 += x2, y1 += y2)
      {
        if (CELL_IS_SOLID(&map->item[f2i(y1)][f2i(x1)])) return FALSE;
      }
    }
  }
  return TRUE;
