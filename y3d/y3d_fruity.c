    ...
    if (POLYCLIP_IS_VISIBLE(p))
    {
      for (k = n; k--;)
      {
        ...
        if (VERTEX_IS_OUTSIDE(vertex))
        {
          ...
          for (k = n; k--; p[k]->d = p[k]->x + p[k]->z); n = polycliper(a, p, n, &fruity->vpool); // Left
          for (k = n; k--; a[k]->d = a[k]->z - a[k]->x); n = polycliper(p, a, n, &fruity->vpool); // Right
          for (k = n; k--; p[k]->d = p[k]->y + p[k]->z); n = polycliper(a, p, n, &fruity->vpool); // Top
          for (k = n; k--; a[k]->d = a[k]->z - a[k]->y); n = polycliper(p, a, n, &fruity->vpool); // Bottom
...
{
  int lt_i, lt_x, lt_xx, lt_l, lt_ll;
  int rt_i, rt_x, rt_xx, rt_l, rt_ll;
  int x1, x2, i;
...
#undef  AFFINE
    ...
    lt_x += lt_xx; lt_l += lt_ll;
    rt_x += rt_xx; rt_l += rt_ll;
  }
}
