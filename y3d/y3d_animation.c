...
void animation_tick(animation_t* a)
{
  if (a->length && a->nloops)
  {
    ...
    if (a->pos < a->start)
    {
      if (--a->nloops) a->pos += a->length; else a->pos = a->start;
    }
    if (a->pos >= a->end)
    {
      if (--a->nloops) a->pos -= a->length; else a->pos = a->end - 1;
    }
    if (a->nloops > (LOOP_FOREVER - 4)) a->nloops = LOOP_FOREVER;
  }
}
...
/*
** Name: keyframe_init
** Desc: NOT READY FOR USE!
** Note: Incomplete. This function is for a uncomming feature that will
**       support "flythrough" scenes & scripted entities movements.
*/
void keyframe_init(keyframe_t* dst, int i, keyframe_t* frames, int n)
{
  keyframe_t* a = &frames[f2i(i)];
  keyframe_t* b = &frames[f2i(i) + 1];

  i = fixfrac(i);

  dst->x = FIXLERP(a->x, a->x, i);
  dst->y = FIXLERP(a->y, a->y, i);
  dst->z = FIXLERP(a->z, a->z, i);
  dst->r = FIXLERP(a->r, a->r, i);
  dst->t = FIXLERP(a->t, a->t, i);
  dst->p = FIXLERP(a->p, a->p, i);
}
